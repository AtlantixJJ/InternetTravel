	...
	public:
		...
		int request(pair<int,int> OD,vector<int> car_set)//规划新的OD请求应归于哪辆集合car_set中的车比较合适，并将其规划如车的路线中，并返回车的ID
		{
			...
			//欧几里得规划裁剪
			{
				...
				//根据距离保留前K个
				vector<int> new_set;
				for(int i=0;i<Global_Scheduling_Vehicles_Per_Request&&i<ans.size();i++)
		...
		void del(int car_id,int node_id)
		{
			...
			for(int i=0;i<car.ODlist.size();i++)
			{
				...
				{
					if(i&1){car.pop(i);car.pop(i-1);car.solve_value(car.LastOrderList);}
					else car.ODlist[i]=-1;
				}
			}
		}
		...
		struct vehicle
		{
			...
			void pop(int id=0)//删除ODlist中第id号元素
			{
				...
				while(id+1<ODlist.size())
				{
					...
					swap(pos_to_ODlist[id],pos_to_ODlist[id+1]);
					id++;
				}
				ODlist.pop_back();
				...
				while(1)
				{
					...
					for(i=0;pd&&i<n;i++)
						if(p[i]&1)
						{
							pd=false;
							for(j=0;!pd&&j<i;j++)
								if(p[j]==p[i]-1)pd=true;
						}
					if(pd)
					{
						for(i=0;ODlist[p[i]]==-1;i++);
						now=pos_to_ODlist[p[i]]+offset;
						for(;i<n-1;i=j)
						{
							for(j=i+1;j<n&&ODlist[p[j]]==-1;j++);
							if(j<n)now+=dist.a[p[i]][p[j]];
						}
						if(now<ans)
						{
							ans=now;
							re.clear();
							for(i=0;i<n;i++)re.push_back(p[i]);
						}
					}
...
/// Setup GPTree given edge and node file path. If index file is loaded
extern void tree_setup(const char *edge_file, const char *node_file, const char *index_file, bool is_load) {
	memcpy(Edge_File, edge_file, strlen(edge_file)+1);
	memcpy(Node_File, node_file, strlen(node_file)+1);
	...
	if(is_load) {
		...
		printf("Load from index file: %s\n", index_file);
		load(index_file);
	} else {
		printf("Build index file from scratch.\n");
		tree.build();
	}
}

/// To avoid long computation in index building
